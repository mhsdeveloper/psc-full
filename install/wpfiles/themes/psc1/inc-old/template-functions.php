 ...
 * @return array
 */
function primary_source_one_body_classes( $classes ) {
...
add_filter( 'body_class', 'primary_source_one_body_classes' );
 ...
 * @return array
 */
function primary_source_one_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'primary_source_one_comment_form_defaults' );
 ...
 * @return bool
 */
function primary_source_one_can_show_post_thumbnail() {
 ...
 * @return int
 */
function primary_source_one_get_avatar_size() {
	return 60;
}
 ...
 */
function primary_source_one_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Name of current post. */
		esc_html__( 'Continue reading %s', 'primarysourceone' ),
 ...
 */
function primary_source_one_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . primary_source_one_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'primary_source_one_continue_reading_link_excerpt' );

/**
 * Create the continue reading link.
 */
function primary_source_one_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . primary_source_one_continue_reading_text() . '</a></div>';
	}
}

// Filter the excerpt more link.
add_filter( 'the_content_more_link', 'primary_source_one_continue_reading_link' );

if ( ! function_exists( 'primary_source_one_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function primary_source_one_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'primarysourceone' ) : $title;
	}
}
add_filter( 'the_title', 'primary_source_one_post_title' );
 ...
 * @return string
 */
function primary_source_one_get_icon_svg( $group, $icon, $size = 24 ) {
	return PSC_1_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function primary_source_one_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? primary_source_one_get_icon_svg( 'ui', 'arrow_right' ) : primary_source_one_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? primary_source_one_get_icon_svg( 'ui', 'arrow_left' ) : primary_source_one_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'primary_source_one_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function primary_source_one_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'primary_source_one_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'primary_source_one_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'primary_source_one_generate_css' ) ) {
	...
	return primary_source_one_generate_css( // @phpstan-ignore-line.
 ...
 */
function primary_source_one_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 */
function primary_source_one_password_form( $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'primarysourceone' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'primarysourceone' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'primarysourceone' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'primarysourceone' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'primary_source_one_password_form' );
 ...
 * @return array
 */
function primary_source_one_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'primary_source_one_get_attachment_image_attributes', 10, 3 );
