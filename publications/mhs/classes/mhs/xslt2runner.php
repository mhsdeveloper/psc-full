	...
	class XSLT2Runner {
		...
		public function setFilename($filename){
			...
			if(!is_readable($filename)){
				$this->Config->Messenger->error(new \Exception("Unable to read i/o xml file for xslt: " . $filename));
				$this->Config->Messenger->userError("Unable to read i/o xml file for xslt: " . $filename);
		...
		public function runShellCMD($saxon_jar = false, $xslt_file = false){
			...
			if(!empty($stderr)) {
				$this->Config->Messenger->userError("XSLT error: " . $stderr);
				$this->Config->Messenger->error(new \Exception("XSLT error: " . $stderr));
				return false;
			}

			if(false === file_put_contents($this->outputFilename, $stdout)) {
				$this->Config->Messenger->error(new \Exception("Unable to file_put_contents for XSLT to file: " . $this->outputFilename));
				$this->Config->Messenger->userError("Unable to save the XML as file: " . $this->outputFilename);
				return false;
			}
