...
<!-- Transforms TEI ptr element to html a (link) element. -->
	<xsl:template match="tei:XXpersRef" priority="99">
...
<!-- Transforms TEI ptr element to html a (link) element. -->
	<xsl:template match="tei:XXptr[@target]" priority="99">
			<xsl:choose>
...
<!-- Transforms TEI figure element to html img element. -->
	...
	<xsl:template match="tei:XXfigure[tei:graphic[@url]]" priority="99">
		<xsl:copy>
...
<!-- Transforms TEI figure/head to HTML figcaption -->
	<xsl:template match="tei:XXfigure/tei:head">
		<figcaption><xsl:apply-templates/></figcaption>
	</xsl:template>


	<xsl:template match="tei:XXpb">
