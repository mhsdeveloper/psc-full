...
<!-- Transforms TEI ref element to html a (link) element. -->
	<xsl:template match="tei:ref[@target]" priority="99">
		<xsl:variable name="linkPrefix">
			<xsl:choose>
				<xsl:when test="contains(@target, 'st')"><xsl:value-of select="$stLinkPrefix"/></xsl:when>
				<xsl:when test="@type = 'terms'"><xsl:value-of select="$stLinkPrefix"/></xsl:when>
				<xsl:when test="@type = 'repository'"><xsl:value-of select="$stLinkPrefix"/></xsl:when>
				<xsl:when test="@type = 'short-title'"><xsl:value-of select="$stLinkPrefix"/></xsl:when>
				<xsl:otherwise><xsl:value-of select="$docLinkPrefix"/></xsl:otherwise>
			</xsl:choose>
		</xsl:variable>

		<xsl:variable name="linkClass">
			<xsl:choose>
				<xsl:when test="contains(@target, 'st')">completion</xsl:when>
				<xsl:when test="@type = 'terms'">completion</xsl:when>
				<xsl:when test="@type = 'repository'">completion</xsl:when>
				<xsl:when test="@type = 'short-title'">completion</xsl:when>
				<xsl:when test="@type = 'figurePage'">scanPage</xsl:when>
				<xsl:otherwise>docLink</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:variable name="linkType">
			<xsl:choose>
				<xsl:when test="$linkClass = 'docLink'"></xsl:when>
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="contains(@target, '.jpg')"></xsl:when>
						<xsl:when test="@type">/<xsl:value-of select="@type"/></xsl:when>
						<xsl:when test="contains(@target, 'st')">/short-title</xsl:when>
		...
		<xsl:variable name="target">
			<xsl:choose>
				<xsl:when test="contains(@target, '-fn-')"><xsl:value-of select="substring-before(@target, '-fn-')" /></xsl:when>
				<xsl:when test="contains(@target, 'n')"><xsl:value-of select="substring-before(@target, 'n')" /></xsl:when>
				<xsl:otherwise><xsl:value-of select="@target"/></xsl:otherwise>
			</xsl:choose>
		</xsl:variable>

		<xsl:variable name="linkJumpID">
			<xsl:choose>
				<xsl:when test="contains(@target, 'n')">#<xsl:value-of select="@target"/></xsl:when>
		...
		<xsl:variable name="anchorType">
			<xsl:choose>
				<xsl:when test="@type = 'figurePage'">
					<xsl:choose>
						<xsl:when test="./@mhs:scanPage">scanPage</xsl:when>
						<xsl:otherwise>unavailableScan</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>normalLink</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:choose>
			<xsl:when test="$anchorType = 'scanPage'">
				<xsl:element name="a" namespace="">
					<xsl:attribute name="href"><xsl:value-of select="$baseURL"/>volume/<xsl:value-of select="substring-before(./@mhs:scanPage, 'p')"/>/pageid/<xsl:value-of select="substring-before(./@mhs:scanPage, '.jpg')"/></xsl:attribute>
				...
				<xsl:element name="a" namespace="">
					<xsl:attribute name="href"><xsl:value-of select="$scansLinkPrefix"/><xsl:value-of select="./@mhs:scanPage"/><xsl:value-of select="$linkType"/><xsl:value-of select="$linkJumpID"/></xsl:attribute>
					<xsl:attribute name="class"><xsl:value-of select="$linkClass"/></xsl:attribute>
					[image]
				</xsl:element>
			</xsl:when>
			...
			<xsl:when test="$anchorType = 'unavailableScan'">
				<xsl:element name="span" namespace="">
					<xsl:attribute name="title">Image unavailable due to permissions.</xsl:attribute>
					<xsl:attribute name="class">unavailable</xsl:attribute>
					<xsl:value-of select="string(.)"/>[unavailable]
				</xsl:element>
			</xsl:when>
			...
			<xsl:otherwise>
				<xsl:element name="a" namespace="">
					<xsl:attribute name="href"><xsl:value-of select="$linkPrefix"/><xsl:value-of select="$target"/><xsl:value-of select="$linkType"/><xsl:value-of select="$linkJumpID"/></xsl:attribute>
					<xsl:attribute name="class"><xsl:value-of select="$linkClass"/></xsl:attribute>
					<xsl:choose>
						<xsl:when test="contains(@target, 'st')"><xsl:attribute name="data-type">short-title</xsl:attribute></xsl:when>
						<xsl:otherwise><xsl:attribute name="data-type"><xsl:value-of select="@type"/></xsl:attribute></xsl:otherwise>
					</xsl:choose>
					<xsl:call-template name="rendition"/>
					<xsl:apply-templates/>
					<xsl:choose>
						<xsl:when test="string(.) = ''">[<xsl:value-of select="@target"/>]</xsl:when>
					</xsl:choose>
				</xsl:element>
			</xsl:otherwise>
